...
def zero_out_random_neurons(test_data, drop_ratio=0.05, fake_positions=None):
    ...
    if fake_positions is not None and len(fake_positions) > 0:
        all_indices = [i for i in all_indices if i not in fake_positions]
        
    n_to_drop = int(len(all_indices) * drop_ratio)
    
    rng = np.random.default_rng(42)
    dropped_neurons = rng.choice(all_indices, size=n_to_drop, replace=False)
    
    modified_test_data = test_data.copy()
    modified_test_data[:, dropped_neurons] = 0.0
    
    print(f"  [Robustness Test] Dropped {n_to_drop} neurons: {dropped_neurons}")
...
for SESSION_FILE in sessions:
    ...
    for training_mode, adv in [("clean", False), ("adversarial", True)]:
        ...
        if session_name not in scores[mode]:
            scores[mode][session_name] = {}
        scores[mode][session_name]["normal"] = r2_normal
        scores[mode][session_name]["robust"] = r2_robust
        
        print(f"Normal R² = {r2_normal:.4f} | Robust (Dropped) R² = {r2_robust:.4f}")
    print()
    print("=" * 60)
    print("Decoder Results (Normal vs Robust)")
    print(f"session = {session_name}")
    print("=" * 60)
    for mode in scores:
        print(f"\nTraining Mode: {mode}")
        for sess, res in scores[mode].items():
            print(f"  {sess} -> Normal R²: {res['normal']:.4f} | Robust R²: {res['robust']:.4f}")
